...
export async function completeDriverMagicLinkSignup(uid, { name, email, carModel, plate, vehicleType }) {
  const row = {
    id: uid, name, email, car_model: carModel, plate, vehicle_type: vehicleType,
  ...
  if (error) throw error;
  return { uid, name, email, carModel, plate, vehicleType, earningsToday: 0, rating: 5.0, audioRecordingEnabled: false, videoRecordingEnabled: false, backgroundCheckStatus: "pending", pendingApproval: false };
}
